...
d3.csv(dataPath)
        .then(function (data) {
                ...
                var energyAvgRegion = d3.nest()
                        ...
                        .rollup(function (v) { return d3.mean(v, function (d) { return Math.floor(d.amount); }); })
                        .entries(data);
                console.log(JSON.stringify(energyAvgRegion));
                for (let i = 0; i < energyAvgRegion.length; i++) {
                        console.log(Math.floor(energyAvgRegion[i].value));
                }
